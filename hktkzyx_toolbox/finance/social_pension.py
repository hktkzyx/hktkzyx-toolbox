...
def get_predicted_personal_balance(current_balance: float,
    ...
    if (payments.size != retire_age - current_age and payments.size != 1):
        raise ValueError(
            '`payments` size should equal `reture_age-current_age` or 1')
    if (predicted_rates.size != retire_age - current_age
            and predicted_rates.size != 1):
        raise ValueError(
            '`predicted_rates` size should equal `reture_age-current_age` or 1'
        )
...
def get_predicted_salary_factor(current_salary_factor: float,
                                current_accumulated_years: int,
                                salaries,
                                social_mean_salaries,
                                predicted_years: Optional[int] = None):
    """预测退休时平均工资指数
